...
export async function ensureSchema() {
  ...
  // Staff accounts for the board's own login system. Passwords are bcrypt
  // hashed. Roles: helper, moderator, community_manager, admin, owner.
  // The first owner is seeded via OWNER_USERNAME / OWNER_PASSWORD env vars
  ...
  // Presence tracking — each browser pings this every 30s while the page
  // is open. Anyone not seen in 90s is treated as offline.
  await db.execute(`
    CREATE TABLE IF NOT EXISTS presence (
      sessionId TEXT PRIMARY KEY,
      username TEXT NOT NULL,
      role TEXT NOT NULL,
      currentTab TEXT NOT NULL DEFAULT 'bugs',
