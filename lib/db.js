...
export async function ensureSchema() {
  ...
  // Tracks which Discord forum thread a bug was imported from, so the same
  // thread is never imported twice even across syncs.
  try {
    await db.execute(`ALTER TABLE bugs ADD COLUMN discordThreadId TEXT`);
  } catch (e) {
    // column already exists — fine
  }
