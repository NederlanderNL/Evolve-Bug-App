...
import { getDb, ensureSchema, checkPasscode, isVoterWhitelisted, uid } from "./db";
...
// Voting requires a name on the VOTE_WHITELIST (see lib/db.js). Votes are
// recorded per-voter server-side (in the suggestion_votes table), so a
// person's vote follows their name across devices/browsers and can be
// switched or retracted — sending { type: null } retracts it.
export function createVoteHandler(table) {
  async function POST(req, { params }) {
    await ensureSchema();
    const body = await req.json();
    const rawName = (body.voterName || "").trim();
    if (!rawName) {
      return NextResponse.json({ error: "Name is required to vote" }, { status: 400 });
    }
    if (!isVoterWhitelisted(rawName)) {
      return NextResponse.json({ error: "This name isn't on the staff voting list" }, { status: 403 });
    }
    const voter = rawName.toLowerCase();
...
// Looks up which way a given name has voted across all suggestions, so the
// UI can restore their vote state on any device once they enter their name.
export function createMyVotesHandler(table) {
  async function GET(req) {
    await ensureSchema();
    const { searchParams } = new URL(req.url);
    const rawName = (searchParams.get("voter") || "").trim();
    if (!rawName) return NextResponse.json({});
    const voter = rawName.toLowerCase();
    const votesTable = `${table}_votes`;
    const db = getDb();
    const res = await db.execute({
