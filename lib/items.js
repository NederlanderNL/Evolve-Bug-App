...
export function createCollectionHandlers(table) {
  ...
  async function POST(req) {
    ...
    // Check for duplicates against existing entries before inserting.
    // Pass ?force=true to bypass this check if staff are sure it's unique.
    if (!body.force) {
      const existingRes = await db.execute(`SELECT title, description, reporter, reportedAt FROM ${table}`);
      const candidate = { title: body.title.trim(), description: (body.description || "").trim() };
      const result = findDuplicate(candidate, existingRes.rows);
      if (result) {
...
export function createItemHandlers(table) {
  async function PATCH(req, { params }) {
    ...
    if (body.status !== undefined) {
      ...
      // Track when an item is marked fixed so the 2-week cleanup is based
      // on fix date, not report date. Clear it if reopened.
      if (body.status === "fixed") {
        sets.push("fixedAt = ?");
        args.push(new Date().toISOString());
      } else {
        sets.push("fixedAt = NULL");
      }
    }
